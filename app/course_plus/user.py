...
def send_auth_code():
    ...
    r = requests.get("http://www.mebox.top/index.php/Home/SMS/sendCode?phonenumber="+ phone +"&coursePlus=1").json()
    if r["result"] == 1:
...
def register():
    ...
    r = requests.get("http://www.mebox.top/index.php/Home/SMS/verifyCode?phonenumber="+ phone +"&verifyCode="+verifyCode).json()
    if r["result"] == 0:
